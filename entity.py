...
# --- Genetic crossover between two parent neurons ---
def crossover(p1, p2):
    child = Neuron(Bird(r.randint(100, 400)))

    # choose random weight from parents
...
# --- Random mutation for neuron weights ---
...
# --- Neuron class (AI controller for one bird) ---
class Neuron():
    def __init__(self, bird):
        self.bird = bird

        # random weights for inputs
    ...
    # update neuron based on game state
    def update(self, tubes):
        self.bird.update()

        # find next tube in front of bird
        next_tube = None
        for tube in tubes:
        ...
        self.x3 = (next_tube.x - self.bird.x) / WIDTH # tube distance
        self.x4 = (next_tube.space + SPACE / 2) / HEIGHT # tube gap
...
class Bird():
    ...
    def update(self):
        self.vy += GRAVITY
        self.y += self.vy
        self.fitness += 0.01  # small reward for staying alive

    # make bird jump
    def jump(self):
        self.vy = JUMP_STRENGTH

    # check for collision with tubes or ground
    ...
    # draw bird with small details
    def draw(self, screen):
        cx = int(self.x + BIRD_SIZE / 2)
        cy = int(self.y + BIRD_SIZE / 2)

        # create transparent surface for bird
...
class Tube():
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(
            screen,
            DARK_GREEN,
            (self.x, self.y + self.space + SPACE, TUBE_WIDTH, TUBE_HEIGHT - self.y - SPACE - self.space)
        )
